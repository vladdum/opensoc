...
module conv1d (
  ...
  logic [31:0] wr_remaining_q;  // output elements yet to write
  logic [31:0] pe_result_q;     // captured PE result for write
  ...
  // shift_load: assert when read data arrives and we're in RD_WAIT
  ...
  assign vregs[0] = signed'(dma_rdata_i[7:0]);   // incoming sample
  for (genvar i = 1; i < MAX_KERNEL; i++) begin : gen_vregs
    assign vregs[i] = shift_out[i-1];             // old reg[i-1]
  end
        ...
        case (ctrl_addr_i[9:0])
          ...
          default: begin
            ...
            if (ctrl_addr_i[9:0] >= 10'h020 && ctrl_addr_i[9:0] <= 10'h05C) begin
              ...
              widx = (32'(ctrl_addr_i[9:2]) - 32'h08);  // (offset/4) - 8 = index
              if (widx < MAX_KERNEL)
    ...
    case (state_q)
      ...
      RD_WAIT: begin
        if (dma_rvalid_i) begin
          // Kernel is full when fill_count (after this read) >= kernel_size_q
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      // SOFT_RESET: clear status and working registers; leave config intact
      if (soft_reset) begin
        busy_q         <= 1'b0;
        done_q         <= 1'b0;
        fill_count_q   <= 32'd0;
      ...
      case (state_q)
        IDLE: begin
          if (go && length_q != 32'd0) begin
            ...
            // Same/zero-pad: pre-fill (KERNEL_SIZE-1) virtual zeros so the
            // first real read immediately produces an output.
            fill_count_q   <= same_q ? (kernel_size_q - 32'd1) : 32'd0;
            wr_remaining_q <= same_q ? length_q
                                     : (length_q - kernel_size_q + 32'd1);
          end else if (go) begin
            // length == 0: immediately done, no DMA
            done_q <= 1'b1;
          end
        end
        ...
        RD_WAIT: begin
          if (dma_rvalid_i) begin
            // Capture PE result computed combinationally from incoming sample
            pe_result_q  <= pe_result_comb;
            // Advance fill counter and source pointer
